...
from .order_service import (
    parse_decimal as os_parse_decimal,
    parse_optional_decimal as os_parse_optional_decimal,
    parse_tx_date as os_parse_tx_date,
    get_fx as os_get_fx,
    get_position_map as os_get_position_map,
    build_settlement_options as os_build_settlement_options,
    is_multiple_of_unit as os_is_multiple_of_unit,
    get_latest_price_for_date as os_get_latest_price_for_date,
    calculate_cost as os_calculate_cost,
    product_kind as os_product_kind,
    to_execution_price as os_to_execution_price,
    validate_and_calculate_order as os_validate_and_calculate_order,
)
...
def make_app(client=None, product_collection=None, currency_prices=None, order_database=None):
    ...
    def new_transaction():
        ...
        parse_decimal = os_parse_decimal
        parse_optional_decimal = os_parse_optional_decimal
        parse_tx_date = os_parse_tx_date

        def get_fx(from_currency: str, to_currency: str) -> float:
            return os_get_fx(currency_prices, from_currency, to_currency)

        get_position_map = os_get_position_map
        build_settlement_options = os_build_settlement_options
        is_multiple_of_unit = os_is_multiple_of_unit
        get_latest_price_for_date = os_get_latest_price_for_date
        calculate_cost = os_calculate_cost
        product_kind = os_product_kind
        to_execution_price = os_to_execution_price
        ...
        def validate_and_calculate_order():
            return os_validate_and_calculate_order(
                portfolio=selected_portfolio,
                product=product,
                inactive_product=inactive_selected_product,
                template=selected_template,
                order_type=selected_order_type,
                entered_amount=entered_amount,
                entered_price=entered_price,
                entered_tx_date=entered_tx_date,
                settlement_currency=selected_settlement_currency,
                settlement_balance=selected_settlement_balance,
                allowed_settlement_currencies=allowed_settlement_currencies,
        ...
        if request.method == "POST":
            ...
            if effective_action in {"draft", "confirm", "submit"}:
                ...
                try:
                    ...
                    payload_for_draft["price"] = entered_price
            ...
            try:
                if product:
                    ...
                    if selected_order_type == "MARKET":
                        used_price, used_price_date = current_market_price, current_market_price_date
                    else:
