...
class TransactionManager:
    def __init__(self):
        ...
        self.templates = {
            ...
            TransactionTemplate.REDEMPTION: self.redemption_template,
        }
    ...
    def create_transaction(self, transaction_date, portfolio_id, template, account_id, **kwargs):
        alias_to_template = {
            'AFLOSSING': TransactionTemplate.REDEMPTION,
            'REDEMPTION': TransactionTemplate.REDEMPTION,
            'SELL': TransactionTemplate.SELL,
            'BUY': TransactionTemplate.BUY,
            'DIVIDEND': TransactionTemplate.DIVIDEND,
            'PURCHASE': TransactionTemplate.BUY,
            'SALE': TransactionTemplate.SELL,
        }
        ...
        if template not in self.templates:
            ...
            if isinstance(template, str):
                ...
                normalized_template = alias_to_template.get(template_upper)
                if normalized_template is None:
                    normalized_template = TransactionTemplate.__members__.get(template_upper)
                if normalized_template is None:
                    normalized_template = next(
                        (member for member in TransactionTemplate if member.value == template),
                        None,
                    )
            else:
                template_name = getattr(template, "name", None)
                template_value = getattr(template, "value", None)

                if template_name:
                    template_name_upper = str(template_name).upper()
                    normalized_template = alias_to_template.get(template_name_upper)
                    if normalized_template is None:
                        normalized_template = TransactionTemplate.__members__.get(template_name_upper)
                if normalized_template is None and template_value is not None:
                    value_upper = str(template_value).upper()
                    normalized_template = alias_to_template.get(value_upper)
                if normalized_template is None and template_value is not None:
                    normalized_template = next(
                        (member for member in TransactionTemplate if member.value == template_value),
                        None,
                    )
...
#toegevoegd       
    ...
    def redemption_template(self, transaction_date, portfolio_id, account_id, product_id, amount, price=1.0, cost=0.0):
        transaction = Transaction(transaction_date, portfolio_id, account_id)
        security_movement = SecurityMovement(
            transaction=transaction,
            product_id=product_id,
